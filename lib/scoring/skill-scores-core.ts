...
import type { Database, SkillTrend } from "@/types/database";

const TREND_CHANGE_THRESHOLD = 3;

/**
 * הליבה הטהורה של "מדוד שוב" (סעיף 29 במפרט) — מקבלת קליינט Supabase מוכן
 * (תמיד עם הרשאות service_role, כי RLS חוסם כתיבה ל-student_skill_scores)
 * במקום ליצור אחד בעצמה. כך אפשר להשתמש בה גם משרת ה-Next.js (דרך
 * skill-scores.ts, המוגן ב-"server-only") וגם מסקריפטים עצמאיים כמו
 * scripts/seed-demo.ts, שאינם רצים תחת תנאי ה-build של Next ולכן לא יכולים
 * לייבא קובץ המוגן ב-"server-only".
 */
export async function recomputeStudentSkillScoresWithClient(
