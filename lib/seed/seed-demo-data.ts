/**
 * ליבת זריעת נתוני הדמו: מורה אחד, כיתה אחת, 8 תלמידים, 12 טקסטים עם שאלות,
 * ונתוני קריאה היסטוריים לארבעה תלמידים (לצורך הדגמת דוחות/גרפים עם נתונים אמיתיים).
 *
 * מקבלת קליינט Supabase מוכן (תמיד עם הרשאות service_role, כי RLS חוסם כתיבה
 * ישירה לחלק מהטבלאות) במקום ליצור כזה בעצמה - כך אפשר להריץ אותה גם מסקריפט
 * CLI מקומי (scripts/seed-demo.ts) וגם מ-API route בשרת המופרס (ראו
 * app/api/admin/seed-demo/route.ts) לזריעה חד-פעמית ללא צורך בגישת רשת ישירה
 * מהמכונה שמריצה את קוד ה-AI.
 *
 * אידמפוטנטית: אפשר להריץ אותה כמה פעמים בלי ליצור כפילויות (upsert / בדיקת
 * קיום לפי email/title).
 */
...
async function findUserByEmail(supabase: DB, email: string) {
...
async function ensureTeacher(supabase: DB) {
  let user = await findUserByEmail(supabase, DEMO_TEACHER.email);
...
async function ensureClass(supabase: DB, teacherId: string) {
...
async function ensureStudent(supabase: DB, student: { username: string; full_name: string }, classId: string) {
...
async function ensureQuestionsForText(
  supabase: DB,
  textId: string,
  title: string,
...
async function ensureTexts(supabase: DB, teacherId: string) {
  ...
  for (const demoText of DEMO_TEXTS) {
    ...
    if (existing) {
      ...
      await ensureQuestionsForText(supabase, existing.id, demoText.title, skillIdByKey);
      continue;
    }
    ...
    await ensureQuestionsForText(supabase, created.id, demoText.title, skillIdByKey);
  }
}
...
async function ensureAssignmentAndHistory(supabase: DB, teacherId: string, classId: string) {
  ...
  const { data: inferenceSkill } = await supabase.from("skills").select("id").eq("key", "inference").single();
  ...
  for (const [username, pattern] of Object.entries(HISTORICAL_PATTERNS)) {
    const user = await findUserByEmail(supabase, usernameToStudentEmail(username));
    if (!user) continue;
...
export interface SeedResult {
  ...
  studentUsernameExample: string;
  studentPassword: string;
  studentCount: number;
}

export async function seedDemoData(supabase: DB): Promise<SeedResult> {
  console.log("🌱 מתחיל זריעת נתוני דמו ל-ReadWise AI...\n");

  const teacher = await ensureTeacher(supabase);
  const classId = await ensureClass(supabase, teacher.id);

  for (const student of DEMO_STUDENTS) {
    await ensureStudent(supabase, student, classId);
  }

  await ensureTexts(supabase, teacher.id);
  await ensureAssignmentAndHistory(supabase, teacher.id, classId);

  console.log("\n✅ סיום זריעת נתוני הדמו.");

  return {
    teacherEmail: DEMO_TEACHER.email,
    teacherPassword: DEMO_TEACHER.password,
    studentUsernameExample: DEMO_STUDENTS[0].username,
